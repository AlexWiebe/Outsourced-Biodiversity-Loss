...
### DESCRIPTION:
### Creates a modified version of Fig 2 that shows the ratio between international and
...
BLUE <- "#25325C"
  ...
  BLACK <- "#202020"
...

    {
      ...
      plt <- ggplot(data = d, aes(x = ratio, y = country.name,
                                  fill = BLUE)) +
        ...
        geom_shadowtext(
          data = subset(d, ratio < 250),
          aes(ratio, y = country.name, label = country.name),
          hjust = 0,
          nudge_x = 50,
          color = BLUE,
          bg.color = "white",
        ...
        labs(
          title = "Ratio of International to Domestic \nAttributable Biodiversity Loss"
        ) + 
        theme(
