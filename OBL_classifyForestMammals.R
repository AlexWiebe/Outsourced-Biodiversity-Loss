...
## This script classifies forest-obligate mammal species.
...
my.iucn.key = "" # replace with your IUCN key
mammal_ranges = vect(paste0(getwd(), "/Mammal_rangemaps/IUCN_maps.shp"))

forest.ids = c()
blocksize= 100
blocks = ceiling(length(mammal_ranges)/blocksize)


getHabs = function(hab, start.index){
  for(i in 1:length(hab)){
    hab[[i]] = rl_habitats(id = mammal_ranges$id_no[c(i + start.index - 1)],
                           key = my.iucn.key)
    print(i)
  }
  hab
...
setwd(paste0(getwd(), "/forestobligate_mammal_ids"))

# Can modify the loop indices here to split up runs manually.
for(b in 1:blocks){
  if(b < blocks){
    ...
    indices = c(c(b*blocksize-99) : c(length(mammal_ranges)))
  ...
  for (s in 1:length(hab)){
    ...
    if(s%%100 == 0 | b == blocks){
      ...
      filepath = paste0("forestobligate_mammal_ids_", b, ".csv")
